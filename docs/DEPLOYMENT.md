...
| **The extensions** | `vector`, `pg_trgm`, and `postgis` are opportunistic accelerators. `db.ensure_extensions()` attempts `CREATE EXTENSION IF NOT EXISTS` for each during `pipeline migrate`, but a managed service that lacks one still starts: semantic search falls back to an in-Python cosine sweep, fuzzy-name search to `LIKE` / `difflib`, and geometry exports to GeoJSON plus shapely. Install the extensions where the database provides them, or use [`deploy/postgres/Dockerfile`](../deploy/postgres/Dockerfile) for the full capability set. |
