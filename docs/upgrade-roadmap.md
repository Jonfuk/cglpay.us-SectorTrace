...
- Still open: run `nlp relations` + `queue-claims` on the **source** deployment (the beta box's warehouse is a copy), then the real human review pass (model triage is a starting point, not the record). `waiting_time` / `vacancy_pressure` may still land in `advisory` even at floor 25; the quorum (`MIN_CATEGORIES_READY`) is re-checked against the real decided counts, not lowered pre-emptively.
