...
- Still open: run `nlp relations` + `queue-claims` on the **source** deployment (the beta box's warehouse is a copy), then the real human review pass over the three trainable categories (model triage is the starting sheet, not the record) plus a second reviewer for the double-review sample. `tupe`'s ~27 model-approved positives need ~8 more from the human pass to clear 35.
