...
**D-08 · 034F relation extraction is high-volume and low-precision — 034G is not trainable on it · M — code fix landed 2026-08-30; source re-run + re-triage open**
...
- Still open: **re-run the NLP chain (`nlp relations` at least) on the source deployment**, let it sync here, and re-run the ensemble triage per category. 034G stays gated until that shows a trainable positive count; if it does not, `GATE_CATEGORIES` is redrawn around what the corpus supports.
