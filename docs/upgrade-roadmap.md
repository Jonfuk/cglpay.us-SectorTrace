...
**D-08 · 034F relation extraction is high-volume and low-precision — 034G is not trainable on it · M — open, filed 2026-08-30**
...
- Fix: a 034D/034F precision pass — relation patterns (`ontology/patterns/*.yml`, `CONCEPT_PREDICATE`) that require an affirming predication (a reliance / pressure / transfer verb in a non-negated, non-hypothetical, non-interrogative clause with the subject in it), not concept-phrase co-occurrence; and a review of the deterministic screen's `SCREEN_MAX_SPAN` (800 chars over-fires on committee bullet-blob "sentences"). Then re-run the chain on the **source** deployment (a mirror's warehouse is replaced on sync) and let it arrive here. This is scoped work, not a patch, and 034G stays gated until the same ensemble triage over the rebuilt candidates shows a trainable positive count.
- Not verified by anything yet — the triage tooling that surfaced it (`pipeline/nlp/review_suggest.py`, `review_batch.py`) is itself the QA check for the rebuild.
