...
One row per collecting module, rendered from the in-code registries
(`pipeline/web/datasets.py`, `pipeline/licences.py`) so it cannot drift from
what the code actually does. `pipeline docs-check` fails CI when this block is
stale; `pipeline docs-sync` rewrites it. The prose sections below add the
things a table cannot carry — endpoints, quirks, the exact coverage bound.

<!-- BEGIN GENERATED: source-capability-matrix -->
<!-- Generated by `pipeline docs-sync` from pipeline/docs_matrix.py. Do not edit between the markers; run `pipeline docs-check` in CI. -->

| Module | Source | Evidence layer | Cadence | Public tables | Licence |
| --- | --- | --- | --- | --- | --- |
| `m00_geography` | ONS Open Geography Portal | Reference & geography | Annual boundary vintages | `authorities` | Open Government Licence v3.0 (contains OS data) |
| `m01_procurement` | Find a Tender & Contracts Finder (Cabinet Office / Crown Commercial Service) | Procurement & spend | Continuous; the pre-2020 CSV archive is daily files | `contracts` | Open Government Licence v3.0 |
| `m02_tribunals` | HM Courts & Tribunals Service (via GOV.UK) | Accountability & scrutiny | Continuous | `tribunal_cases`, `eat_cases` | Open Government Licence v3.0 |
| `m03_charity_finance` | Charity Commission for England and Wales | Public finance | Annual filings | `charity_financials`, `charity_accounts_extracts` | Open Government Licence v3.0 |
| `m04_companies` | Companies House | Provider identity & structure | Continuous | `companies`, `company_filings` | Open Government Licence v3.0 |
| `m05_cqc` | Care Quality Commission (syndication API) | Provider identity & structure | Continuous | `cqc_locations`, `cqc_location_reports` | Open Government Licence v3.0 |
| `m06_workforce_census` | NHS Benchmarking Network for NHS England | Workforce benchmarks | Annual | `workforce_census_metrics`, `workforce_census_reports` | NHS England / NHS Benchmarking Network — not OGL |
| `m07_ndtms` | Office for Health Improvement and Disparities | Treatment demand & outcomes | Annual | `ndtms_la_statistics`, `ndtms_publications` | Open Government Licence v3.0 |
| `m08_pfd_reports` | Courts and Tribunals Judiciary | Safety & safeguarding | Continuous | `pfd_reports`, `pfd_documents` | Open Government Licence v3.0 |
| `m09_cdp_documents` | Local Combating Drugs Partnerships | Accountability & scrutiny | Ad hoc | `cdp_documents`, `cdp_document_candidates` | Varies by authority |
| `m10_committee_papers` | Local authority committee-management systems | Accountability & scrutiny | Continuous | `committee_papers`, `committee_paper_candidates` | Varies by authority |
| `m11_public_health_grant` | Department of Health and Social Care | Public finance | Annual | `public_health_grants` | Open Government Licence v3.0 |
| `m12_fingertips` | Office for Health Improvement and Disparities — Fingertips | Treatment demand & outcomes | Varies by indicator, mostly annual | `fingertips_la_values`, `fingertips_indicators` | Open Government Licence v3.0 |
| `m13_la_budgets` | Ministry of Housing, Communities and Local Government | Public finance | Annual | `la_revenue_budgets`, `la_budget_publications` | Open Government Licence v3.0 |
| `m14_annual_reports` | Charity Commission filed annual reports (tracked provider set) | Public finance | Annual | `provider_report_disclosure`, `provider_report_passages` | The charity's own copyright |
| `m15_foi` | mySociety WhatDoTheyKnow and council disclosure logs | Accountability & scrutiny | Ad hoc | `foi_requests`, `foi_request_candidates` | CC BY-SA (mySociety) with OGL v3.0 responses |
| `m16_nhs_jobs` | NHS Jobs (NHS Business Services Authority) | Pay evidence | Continuous | `nhs_job_adverts`, `nhs_job_advert_locations` | Crown copyright, advert content the employer's |
| `m17_statutory_pay_rates` | GOV.UK (Department for Business and Trade) | Pay evidence | Annual, each April | `statutory_pay_rates` | Open Government Licence v3.0 |
| `m18_living_wage` | Living Wage Foundation (Citizens UK) | Pay evidence | Continuous | `living_wage_accreditations` | Living Wage Foundation — charity-published register |
| `m19_data_gov_uk` | data.gov.uk (Central Digital and Data Office) | Reference & geography | Continuous | `data_gov_uk_datasets`, `data_gov_uk_resources` | Open Government Licence v3.0 |
| `m20_gender_pay_gap` | Gender Pay Gap Service (Government Equalities Office) | Pay evidence | Annual, by snapshot date | `gender_pay_gap_reports` | Open Government Licence v3.0 |
| `m21_ons_ashe` | Office for National Statistics — Annual Survey of Hours and Earnings | Pay evidence | Annual | `ons_ashe_observations` | Open Government Licence v3.0 |
| `m22_provider_pay_pages` | Tracked providers' own websites | Pay evidence | Ad hoc, when a page changes | `provider_pay_pages`, `provider_pay_mentions` | The provider's own copyright |
| `m23_sector_universe` | Derived from Companies House, the Charity Commission and CQC | Sector context | Recomputed on each run | `sector_universe` | Open Government Licence v3.0 |
| `m24_council_spend` | Individual local authorities (over-£500 payment data) | Procurement & spend | Monthly, per authority | `council_spend`, `council_spend_files` | Varies by authority |
| `m25_skills_for_care` | Skills for Care — Adult Social Care Workforce Data Set (ASC-WDS) | Workforce benchmarks | Annual | `skills_for_care_estimates`, `skills_for_care_files` | OGL v3.0 (ASC-WDS data, per the data.gov.uk catalogue) |
| `m26_cqc_directory` | Care Quality Commission bulk exports | Provider identity & structure | Weekly | `cqc_providers` | Open Government Licence v3.0 |
| `m27_ndtms_monthly` | National Drug Treatment Monitoring System (OHID) | Treatment demand & outcomes | Monthly, provisional | `ndtms_monthly_statistics` | Open Government Licence v3.0 |
| `m28_sar_reports` | National SAR Library and Safeguarding Adults Boards | Safety & safeguarding | Ad hoc | `sar_documents`, `sar_provider_mentions` | Varies by authority |
| `m29_rough_sleeping` | Ministry of Housing, Communities and Local Government | Comparator (never combined) | Annual, autumn snapshot | `rough_sleeping_snapshot` | Open Government Licence v3.0 |
...
| | |
| --- | --- |
| Sources | Individual local authority websites and their committee-management systems. **Only ModernGov is searched.** CMIS, Democracy and other systems are detected by signature and recorded as unsupported rather than crawled — see the Coverage row |
...
| | |
| --- | --- |
| Source | Individual funders publishing to the 360Giving Data Standard, read via the 360Giving API — a separate, newer REST API (`api.threesixtygiving.org`) from GrantNav's own retired bulk endpoints |
| Endpoints | `https://api.threesixtygiving.org/api/v1/org/<org_id>/grants_received/`, `.../grants_made/`, paginated with `limit`/`offset` |
