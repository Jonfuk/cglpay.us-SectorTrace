...
| Key | None |
| Rate limit | Default (2s/host), conditional requests |
| Notes | The library gives no structured metadata per document — no board name, no publication date, no distribution list, only a title (as submitted, often a bare filename) and a download link, grouped by the year the library filed it under. `sab_name` is read from the document's own text where it names its board plainly ("... Safeguarding Adults Board"), never guessed from the title or the year folder — NULL plus a `parse_failures` row when the pattern is not found. There is no PFD-style "matters of concern" excerpt: judiciary.uk's reports share one template and SAR reports are written by ~150 different boards over a decade with no shared one, so no section is extracted into a public column by pattern-matching. What this module gives instead is the same term-frequency finding aid Module 8 uses and provider mentions across the full text. A title is very often the subject's own name or a chosen pseudonym with nothing in the source to say which, so every title lives only in `restricted_sar_persons`, never a public column. A document is skipped on a later run only once text has actually been extracted from it, or its extension is one this module still cannot read at all — a document recorded with no text is retried rather than treated as settled, because the module's ability to read it can improve after the fact (this is exactly what happened when DOCX support was added: every DOCX read beforehand picked itself up on the very next plain rerun, with no separate backfill command needed). There is no per-document date to filter on with `--since`, so revisiting the full listing every run is the only way to notice additions. Text is read from both PDF and DOCX documents — the library carries a meaningful share of each — the latter with the stdlib-only DOCX parser already built for the document-analysis worker (`pipeline/documents/parsers.py`, zipfile + xml.etree, no extra dependency) rather than adding python-docx for a handful of files. `.doc` (the old binary format) and `.odt` are recorded but not read: rare enough in the corpus not to justify a third parser, and left as a `parse_failures` row rather than guessed at |
