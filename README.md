...
| `m31_temporary_accommodation` | MHCLG temporary accommodation (H-CLIC) | Households placed in temporary accommodation by a council as at the end of each quarter, and the with-children/children breakdown. Reads Table TA1 from the same quarterly workbook Module 30 reads Table A1 from — same evergreen page, same attachment, same discovery code (shared, not duplicated; see the module's docstring) |
| `m32_sab_site_reviews` | Each England Safeguarding Adults Board's own website | The documented exception to Module 28's "one aggregator" rule (`docs/m32-sab-site-crawl.md`): a bounded crawl of each board's site — from the `website_url` Module 28 stores in `safeguarding_adults_boards` — for SARs the board published but never submitted to the National SAR Library. Hybrid gate: a document is auto-ingested only when the link unambiguously says "Safeguarding Adults Review" and its text names that board; anything weaker is a `review_queue` candidate, and byte-identical copies of library documents are skipped. `sab_site_crawls` records what each board yielded |
...
| --- | --- | --- |
| 1 | `m00`, `m02`, `m03`, `m06`, `m08`, `m16`, `m17`, `m18`, `m21`, `m22`, `m25`, `m28`, `m29`, `m30`, `m31` | Independent geography, provider, workforce, pay and comparator sources |
