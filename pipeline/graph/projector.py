...
class GraphProjector:
    ...
    def rebuild(self, clear: bool = False) -> dict[str, int | str]:
        ...
        # A span around the whole rebuild (performance.md:632's "graph
        # projection"), reusing the same numbers `graph.rebuild_completed`
        # already computes below rather than timing anything twice.
        with telemetry.span("graph.rebuild", run_id=run_id, clear=clear) as rebuild_span:
            try:
            ...
            rebuild_span.set_attributes({f"rows_{key}": value for key, value in counts.items()})
            rebuild_span.set_attribute("rows_total", rows_total)
            rebuild_span.set_attribute("row_failures", self._run_row_failures)
            telemetry.histogram(
                "graph.rebuild.duration_seconds", unit="s",
                description="Wall time of one graph rebuild.").record(duration_seconds)
            telemetry.histogram(
                "graph.rebuild.rows", unit="rows",
                description="Rows projected per rebuild.").record(rows_total)
            if self._run_row_failures:
                telemetry.counter(
                    "graph.rebuild.row_failures",
                    description="Rows a rebuild could not project even after bisection, "
                                "queued for graph sync instead.").add(self._run_row_failures)
            self.conn.execute(
    ...
    def sync_delta(self, limit: int = 500) -> dict[str, int]:
        ...
        with telemetry.span("graph.sync_delta", limit=limit,
                            queue_depth_before=queue_depth) as sync_span:
            ...
            # Reusing the numbers the log line above just computed -- queue
            # depth and per-item lag are the metrics performance.md:632 asks
            # for ("projection lag", "queue depth").
            sync_span.set_attributes({
                "batch_size": len(rows), "processed": processed, "failed": failed})
            telemetry.histogram(
                "graph.projection_queue_depth", unit="rows",
                description="Pending graph_projection_queue rows, sampled at the "
                            "start of a sync.").record(queue_depth)
            if failed:
                telemetry.counter(
                    "graph.sync_delta.failures",
                    description="Queued projection items that failed this sync attempt.").add(failed)
            for lag in lag_seconds:
                telemetry.histogram(
                    "graph.projection_lag_seconds", unit="s",
                    description="Time between a queued change and its projection.").record(lag)
