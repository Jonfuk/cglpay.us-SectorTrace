...
def _add_authority(conn, ons_code: str = "E99999001", name: str = "Testshire") -> None:
    ...
    conn.execute(
        ...
        "payload_sha256) VALUES (%s, %s, %s, 'https://register.example', "
...
def test_run_records_an_unreadable_file(httpx_mock, settings, conn):
    ...
    assert conn.execute("SELECT COUNT(*) FROM council_spend").fetchone().values().__iter__().__next__() == 0
...
def test_run_records_no_file_found(httpx_mock, settings, conn):
    ...
    assert conn.execute("SELECT COUNT(*) FROM council_spend_files").fetchone().values().__iter__().__next__() == 0
