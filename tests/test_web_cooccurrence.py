...
def _providers(conn):
    for key, name in [("cgl", "Change Grow Live"), ("tp", "Turning Point")]:
        conn.execute("INSERT INTO providers (provider_key, canonical_name, "
                     "is_target, notes) VALUES (?, ?, 0, NULL)", (key, name))
        conn.execute("INSERT INTO supplier_aliases (alias_raw, supplier_key, "
                     "canonical_name) VALUES (?, ?, ?)", (name, key, name))
    conn.commit()
...
def _document(conn, element_text):
    conn.execute(
        ...
        " 'r/x.pdf', 'application/pdf', 10, 'committee_papers', 'k1', ?)",
        (_NOW, _NOW))
    conn.execute(
        ...
        " 'application/pdf', 'Board minutes', ?, ?)", (_NOW, _NOW))
    conn.execute(
