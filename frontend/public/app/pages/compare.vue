...
import { comparisonMeasures, comparisonPoints } from '~/lib/comparison-chart'
...
interface CompareWorkspace { requestKey: string; authorities: AuthoritiesResponse; providers: ProvidersResponse; compare: CompareEnvelope | null; providerPay: ProviderCompare | null }
...
const requestSignature = computed(() => JSON.stringify({ kind: kind.value, keys: activeKeys.value, pay: payLens.value, blocked: selectionError.value, supported: lenses.value.includes(lens.value) }))
const { data: rawData, pending, error, refresh } = await useAsyncData('public-compare-workspace', async (_app, { signal }) => {
  const f = filters.all()
  const requestKey = requestSignature.value
  ...
  if (selectionError.value || !lenses.value.includes(lens.value) || !selected || (payLens.value && selected < 2)) return { requestKey, authorities, providers: providerList, compare: null, providerPay: null }
  ...
  return { requestKey, authorities, providers: providerList, compare, providerPay }
}, { watch: [requestSignature] })
const data = computed<CompareWorkspace | null>(() => rawData.value?.requestKey === requestSignature.value ? rawData.value : null)
...
const measures = computed(() => comparisonMeasures(lens.value))
const chartRows = computed(() => { const rows = data.value?.compare?.series?.[lens.value]?.rows; return Array.isArray(rows) ? rows : [] })
const presentation = computed(() => { const value = filters.get('view'); return typeof value === 'string' && value ? value : 'chart' })
const chartOffset = computed(() => { const value = filters.get('chart_offset'); return typeof value === 'string' && /^\d+$/.test(value) ? Number(value) : 0 })
const peerPoints = (peer: string) => comparisonPoints(chartRows.value, lens.value, peer)
const chartUnit = computed(() => ['contracts', 'provider_contracts'].includes(lens.value) ? 'Published notices' : 'GBP')
const selectedChartRecord = computed(() => { const index = filters.get('record_index'); if (filters.get('record_scope') !== lens.value || typeof index !== 'string' || !/^\d+$/.test(index)) return undefined; return JSON.stringify(chartRows.value[Number(index)]) === filters.get('record_snapshot') ? Number(index) : undefined })
function peerName(peer: string) { return kind.value === 'provider' ? providerName(peer) : authorityOptions.value.find(row => row.ons_code === peer)?.name ?? peer }
async function inspectChart(index: number) { await filters.setAll({ ...filters.all(), record_scope: lens.value, record_index: String(index), record_snapshot: JSON.stringify(chartRows.value[index]) }); await nextTick(); document.querySelector<HTMLElement>('.st-inspector-heading')?.focus() }
function chartAnnotation(payload: CompareSeries) { return `Source context: ${JSON.stringify(sourceContext(lens.value, payload))}. Chart group offset ${chartOffset.value}, at most 12 returned records per peer. View ${import.meta.client ? window.location.origin + window.location.pathname : ''}#${route.fullPath}` }
...
function sourceContext(source: string, payload: CompareSeries | ProviderLayer, peer?: string) { return { endpoint: payLens.value ? '/api/v1/provider_compare' : '/api/v1/compare', entity_type: kind.value, selected_keys: activeKeys.value, source, peer: peer ?? null, source_context: { ...payload, rows: undefined, by_provider: undefined }, cross_source_caveat: data.value?.compare?.caveats?.cross_layer ?? data.value?.providerPay?.caveat ?? null, limitations: 'One source is exported at a time. Aggregate provenance is not per-row provenance. Missing observations are not zero. No rankings, ratios or cross-source arithmetic are calculated.' } }
...
<template>
  <section class="space-y-8">
    <header class="st-page-header"><p class="atlas-eyebrow">Research</p><h1>Compare</h1><p>Choose peers of one type and inspect one evidence source at a time. Reporting periods, units and source limitations remain attached to their observations.</p></header>
    ...
    <template v-else-if="!selectionError && (data?.compare || data?.providerPay)">
      <section v-if="data.compare" id="compare-results" class="atlas-section"><div class="atlas-section-head"><h2>Evidence by layer</h2><p>Each table keeps one measure and its provenance context intact.</p></div><StCaveat v-if="data.compare?.caveats?.cross_layer" :text="data.compare?.caveats.cross_layer" /><div class="space-y-6 mt-5"><section v-for="(payload, kind) in series" :key="kind" class="atlas-panel atlas-panel-body space-y-4"><div><h3>{{ seriesNames[kind] ?? kind }}</h3><p class="text-sm opacity-70">{{ kind === 'grant' ? 'Public health grant allocation, as published per financial year.' : kind === 'budget' ? 'What each authority planned to spend, as reported.' : kind === 'treatment' ? 'Fingertips figures, with the confidence interval that belongs to each value.' : kind === 'charity' ? 'Income and expenditure as filed, retaining the supplied financial year end.' : 'Returned notice counts and source aggregates by publication year. Values are not payments or a headline contract total.' }}</p></div><StCaveat v-if="payload.caveat" :text="payload.caveat" /><template v-if="measures.length"><div class="flex gap-2" role="group" aria-label="Comparison presentation"><button type="button" class="atlas-button" :aria-pressed="presentation === 'chart'" @click="filters.set('view', 'chart')">Chart</button><button type="button" class="atlas-button" :aria-pressed="presentation === 'data'" @click="filters.set('view', 'data')">Data</button></div><p v-if="!['chart', 'data'].includes(presentation)" role="status">The saved presentation is unsupported. Choose Chart or Data.</p><template v-if="presentation === 'chart'"><p class="atlas-footnote">Charts show up to 12 source records per peer in each group. All returned records remain in the paged table. Records are not combined across periods or peers.</p><div class="grid gap-6 xl:grid-cols-2"><section v-for="peer in activeKeys" :key="peer" class="min-w-0"><p v-if="!peerPoints(peer).slice(chartOffset, chartOffset + 12).length" role="status">{{ peerName(peer) }}: no source records in this chart group.</p><NuxtErrorBoundary v-else><LazyStComparisonChart :points="peerPoints(peer).slice(chartOffset, chartOffset + 12)" :measures="measures" :selected-index="selectedChartRecord" :title="`${peerName(peer)} ${seriesNames[kind] ?? kind}`" :unit="chartUnit" :annotation="chartAnnotation(payload)" @inspect="inspectChart" /><template #error="{ clearError }"><p role="status">The chart is unavailable. The record table remains available.</p><button type="button" class="atlas-button" @click="clearError">Retry chart</button></template></NuxtErrorBoundary></section></div><nav class="flex gap-2" aria-label="Comparison chart groups"><button type="button" class="atlas-button" :disabled="!chartOffset" @click="filters.set('chart_offset', String(Math.max(0, chartOffset - 12)))">Previous chart group</button><button type="button" class="atlas-button" :disabled="activeKeys.every(peer => peerPoints(peer).length <= chartOffset + 12)" @click="filters.set('chart_offset', String(chartOffset + 12))">Next chart group</button><button v-if="chartOffset" type="button" class="atlas-button" @click="filters.set('chart_offset', undefined)">First chart group</button></nav></template></template><StComparisonRecords :rows="payload.rows" :scope="String(kind)" :title="seriesNames[kind] ?? String(kind)" :context="sourceContext(String(kind), payload)" /><p v-for="peer in activeKeys.filter(id => !(payload.rows ?? []).some(row => row[kind === 'charity' || kind === 'provider_contracts' ? 'provider_key' : 'ons_code'] === id))" :key="peer" role="status">{{ peer }}: no observations were returned in this source.</p><details><summary>Source context and caveats</summary><pre>{{ JSON.stringify({ ...payload, rows: undefined }, null, 2) }}</pre><p>Aggregate provenance can contain up to six source URLs and the latest retrieval across the selection. It is not per-row provenance.</p></details></section></div></section>
      <section v-if="data.providerPay" class="atlas-section"><div class="atlas-section-head"><h2>Provider pay evidence side by side</h2><p>Provider-only comparisons have no common authority axis, so these layers stay as separate lists.</p></div><div class="atlas-panel atlas-panel-body space-y-5"><StCaveat v-if="data.providerPay.caveat" :text="data.providerPay.caveat" /><section v-for="(layer, key) in providerLayers" :key="key"><StCaveat v-if="layer.caveat" :text="layer.caveat" /><p>{{ layer.unit ?? 'Unit not supplied' }}</p><h3>{{ key === 'living_wage' ? 'Living Wage accreditation' : key === 'gender_pay_gap' ? 'Latest gender pay gap filing' : key === 'provider_pay' ? 'Pay published on provider sites' : 'Recent NHS Jobs adverts' }}</h3><div class="grid gap-3 md:grid-cols-2"><div v-for="providerKey in providerKeys" :key="`${key}-${providerKey}`" class="border rounded p-3"><strong>{{ providerName(providerKey) }}</strong><StComparisonRecords :rows="layer.by_provider?.[providerKey]" :scope="`${key}:${providerKey}`" :title="`${providerName(providerKey)} ${seriesNames[key] ?? key}`" :context="sourceContext(String(key), layer, providerKey)" /></div></div></section></div></section>
    </template>
  </section>
</template>
