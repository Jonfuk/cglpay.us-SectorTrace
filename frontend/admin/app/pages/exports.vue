...
const { data, pending, error, refresh } =
  await useAsyncData<ExportsResponse | null>(
    'admin-exports',
    () => api.exports(),
    { default: () => null },
  );
...
function download(path: string | undefined): string {
  return path
    ? `/api/admin/exports/file?path=${encodeURIComponent(path)}`
    : '#';
}
  ...
  try {
    ...
    status.value =
      e instanceof TransportError
        ? e.message
        : 'The export job could not be followed.';
...
async function startExport() {
  ...
  if (!(await dialog.confirm(`Write the ${target.value} export locally?`)))
    return;
  busy.value = true;
...
<template>
  <section class="space-y-6">
    ...
    <UCard>
      <template #header
        ><span class="text-sm font-medium">Generate an export</span></template
      >
      <div class="flex flex-wrap items-end gap-3">
        <label class="text-sm space-y-1">
          ...
          <select
            v-model="target"
            class="border border-black/15 dark:border-white/15 rounded px-2 py-2 bg-transparent"
          >
            <option v-for="name in targets" :key="name" :value="name">
              {{ name }}
            </option>
          </select>
        </label>
        <UButton color="primary" :disabled="busy" @click="startExport">{{
      ...
      <div v-if="currentJob" class="mt-3 text-xs opacity-70">
        Job #{{ currentJob.id }} · {{ currentJob.state }} · started
        {{ when(currentJob.started_at) }}
      </div>
    </UCard>
    ...
    <template v-else>
      <UCard v-if="!files.length">
        <StEmptyState
          title="No exports"
          message="No export files are available yet. Generate one above."
        />
      </UCard>
      <UCard v-for="[group, groupFiles] in groups" :key="group">
        <template #header>
          <div class="flex items-center justify-between gap-3">
            ...
            <span class="text-xs opacity-60">{{
              formatBytes(
                groupFiles.reduce(
                  (sum, file) => sum + Number(file.bytes ?? file.size ?? 0),
                  0,
                ),
              )
            }}</span>
        ...
        <p v-if="staleness(group)?.stale" class="mb-4 text-sm admin-note">
          These files predate the last collection. Re-export before quoting
          figures.
        </p>
        <p v-else-if="staleness(group)" class="mb-4 text-xs opacity-60">
          Written after the last recorded pipeline activity.
        </p>
        ...
        <div class="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-sm">
          <template v-for="file in groupFiles" :key="file.path">
            ...
            >
            <a
              v-if="file.provenance"
              :href="download(file.provenance)"
              class="text-[var(--st-accent)] underline underline-offset-2"
              >Provenance</a
            >
          </template>
