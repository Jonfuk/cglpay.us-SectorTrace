...
test('admin shell boots without console or hydration errors', async ({ page }) => {
  const problems = collectProblems(page)
  await page.goto('./')
  await expect(page.getByRole('link', { name: /SectorTrace Operations/ })).toBeVisible()
...
test('admin hash-history route mounts the claims authoring surface', async ({ page }) => {
  const problems = collectProblems(page)
  await page.goto('./')
